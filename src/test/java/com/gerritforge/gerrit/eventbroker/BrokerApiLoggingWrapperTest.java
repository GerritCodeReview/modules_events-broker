...
public class BrokerApiLoggingWrapperTest {
  ...
  @Mock private MessageAcknowledgement<Event> acknowledgement;
  @Captor private ArgumentCaptor<AckAwareConsumer<Event>> consumerCaptor;
  ...
  public void setUp() {
    objectUnderTest =
        new BrokerApiLoggingWrapper(DynamicItem.itemOf(BrokerApi.class, brokerApi), msgLog);
  }
  ...
  @Test
  public void shouldLogConsumedMessage() {
    objectUnderTest.receiveAsync(TOPIC, consumer);

    verify(brokerApi).receiveAsync(eq(TOPIC), consumerCaptor.capture());
    consumerCaptor.getValue().accept(event, acknowledgement);

    verify(msgLog).log(MessageLogger.Direction.CONSUME, TOPIC, event);
    verify(consumer).accept(event, acknowledgement);
  }
