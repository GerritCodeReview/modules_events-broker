...
public interface MessageAcknowledgement {
   ...
   * concrete broker implementation, callers should invoke {@code ack()} from the same execution
   * context that received the message and must not call it concurrently.
   *
   * <p>Calling {@code ack()} when {@link #isAutoAck()} is {@code true} should fail with {@link
