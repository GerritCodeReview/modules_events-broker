...
public class BrokerApiBoundNotifier {
  ...
  /**
   * Registers a subscriber to be notified whenever a broker plugin binds its {@link BrokerApi}.
   *
   * <p>Registering does not invoke the subscriber. One added while a broker is already bound is
   * only called the next time a broker binds, which may never happen, so a subscriber that can be
   * registered after startup should also consult {@link #boundBrokerApi()} itself once it has
   * registered.
   *
   * <p>Exceptions thrown while notifying a subscriber are logged rather than propagated, so that a
   * failing subscriber cannot disrupt the start of the plugin that triggered the notification.
   */
