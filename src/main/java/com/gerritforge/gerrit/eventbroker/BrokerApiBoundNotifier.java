 ...
 * Consumers register here instead and subscribe once {@link #boundBrokerApi()} is present.
 */
@Singleton
public class BrokerApiBoundNotifier {
   ...
   *
   * <p>A plugin being reloaded, disabled and re-enabled, or replaced by a different broker binds a
   * new instance, so listeners can compare identity to tell whether they are still subscribed to
   * the broker that is currently bound.
   */
