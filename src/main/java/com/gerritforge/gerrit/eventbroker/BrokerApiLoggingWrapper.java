...
public class BrokerApiLoggingWrapper implements BrokerApi {
  ...
  protected ListenableFuture<Boolean> send(
    ...
    resultF.addListener(
        () -> {
          try {
            if (Futures.getDone(resultF)) {
              msgLog.log(direction, topic, message);
            }
          } catch (ExecutionException | RuntimeException | Error e) {
            logSendFailure(
                e instanceof ExecutionException ? e.getCause() : e, direction, topic, message);
          }
        },
  ...
  private static void logSendFailure(
      Throwable throwable, MessageLogger.Direction direction, String topic, Event message) {
  ...
  public void receiveAsync(String topic, AckAwareConsumer<Event> consumer) {
    delegate().receiveAsync(topic, loggingConsumer(topic, consumer));
  }

  @Override
  public void receiveAsync(String topic, String groupId, AckAwareConsumer<Event> consumer) {
    delegate().receiveAsync(topic, groupId, loggingConsumer(topic, consumer));
  }
  ...
  public void receiveAsyncWithPartition(
    ...
    delegate()
        .receiveAsyncWithPartition(topic, partition, groupId, loggingConsumer(topic, consumer));
  }

  private AckAwareConsumer<Event> loggingConsumer(String topic, AckAwareConsumer<Event> consumer) {
