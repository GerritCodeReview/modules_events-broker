 ...
 * and are called back through {@link #onBrokerApiBound()} and {@link #onBrokerApiUnbound()} only
...
public interface BrokerApiPluginListener extends StartPluginListener, StopPluginListener {
  ...
  /** Invoked once a plugin has bound its {@link BrokerApi}. */
  void onBrokerApiBound();

  /** Invoked once the plugin that had bound its {@link BrokerApi} is stopped. */
  void onBrokerApiUnbound();
    ...
    if (providesSubscribedBrokerApi(plugin)) {
      try {
        ...
        log.atSevere().withCause(e).log(
            "Listener failed to react to the BrokerApi of plugin %s being unbound",
            plugin.getName());
      }
    }
  }
