...
Broker implementations can use partition-aware subscriptions through
`BrokerApi.receiveAsyncWithPartition(...)` and can resolve the partition for an
event with `BrokerApi.getPartitionFromEvent(event, topic)`.
